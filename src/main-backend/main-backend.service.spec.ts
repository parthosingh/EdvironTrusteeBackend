...
describe('MainBackendService', () => {
  ...
  describe('createTrustee', () => {
    ...
    it('should throw ConflictException when email already exists', async () => {
      ...
      await expect(service.createTrustee(trusteeInfo)).rejects.toThrowError(
        ConflictException,
      );
    });
    it('should throw BadRequestException for other errors', async () => {
      ...
      await expect(service.createTrustee(info)).rejects.toThrowError(
        BadRequestException,
      );
    });
  });
  ...
  describe('findTrustee', () => {
    ...
    it('should throw Badrequest Exception', async () => {
      jest
        .spyOn(trusteeModel, 'countDocuments')
        .mockRejectedValue(new Error('Test error'));

      const page = 1;
      const pageSize = 5;

      await expect(service.findTrustee(page, pageSize)).rejects.toThrowError(
        NotFoundException,
      );
    });
  });
  ...
  describe('findOneTrustee', () => {
    ...
    it('should throw BadRequestError', async () => {
      ...
      await expect(service.findOneTrustee(trusteeId)).rejects.toThrowError(
        BadRequestException,
      );
    });
  });
  describe('checkSchoolLimit', () => {
    it('should return school limit', async () => {
      ...
      expect(trusteeSchoolModel.countDocuments).toHaveBeenCalledWith({
    ...
    it('should throw BadRequestException', async () => {
      ...
      await expect(service.checkSchoolLimit(trusteeId)).rejects.toThrowError(
        BadRequestException,
      );
    });
  });
  ...
  describe('isKeyUnique', () => {
    ...
    it('should return false if pg_key is not unique', async () => {
      ...
      jest
        .spyOn(trusteeSchoolModel, 'findOne')
        .mockResolvedValueOnce({ pg_key: existingKey });
    ...
    it('should throw BadRequestException for other errors', async () => {
      ...
      await expect(service.isKeyUnique(uniqueKey)).rejects.toThrowError(
        BadRequestException,
      );
    });
  });
  describe('updateSchoolInfo', () => {
    ...
    it('should throw NotFoundException if trustee is not found', async () => {
      ...
      jest.spyOn(trusteeSchoolModel, 'findOne').mockResolvedValueOnce(null);
      await expect(service.updateSchoolInfo(info)).rejects.toThrowError(
        NotFoundException,
      );
    });
    ...
    it('should throw ConflictException for other errors', async () => {
      ...
      jest
        .spyOn(trusteeSchoolModel, 'findOneAndUpdate')
