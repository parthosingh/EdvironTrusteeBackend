...
export class MainBackendController {
  ...
  async createTrustee(
    ...
    try {
      const info: JwtPayload = this.jwtService.verify(token.data, {
  ...
  async findTrustee(
    ...
    const trustee: string = this.jwtService.sign(
      await this.mainBackendService.findTrustee(page, pageSize),
      { secret: process.env.JWT_SECRET_FOR_INTRANET },
    );
    return trustee;
  }
  ...
  async assignSchool(
    ...
    try {
      const data: JwtPayload = this.jwtService.verify(token.token, {
        secret: process.env.JWT_SECRET_FOR_INTRANET,
      }) as JwtPayload;
