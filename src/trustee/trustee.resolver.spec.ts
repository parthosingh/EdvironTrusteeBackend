...
describe('TrusteeResolver', () => {
  ...
  beforeAll(async () => {
    ...
    CommissionModel = mongoConnection.model(Commission.name, CommissionSchema);
  ...
  describe('getSchoolQuery', () => {
    it('should return school data and total pages', async () => {
      ...
      expect(JSON.parse(JSON.stringify(result.schools[0].school_id))).toEqual(
        JSON.parse(JSON.stringify(mockResponse[0].school_id)),
      );
    });
  ...
  describe('createApiKey', () => {
    ...
    it('should throw user not found', async () => {
      const trusteeId = new Types.ObjectId(11);
      const schoolid = new Types.ObjectId(16);
      const password = '123456';
      const context = {
        req: {
