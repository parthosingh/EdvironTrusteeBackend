...
describe('TrusteeService', () => {
  ...
  describe('loginAndGenerateToken', () => {
    it('should return a token for valid credentials', async () => {
      ...
      const result = await service.loginAndGenerateToken(
        'test@example.com',
        'password123',
      );

      expect(result).toEqual({ token: 'mockToken' });
      expect(trusteeModel.findOne).toHaveBeenCalledWith({
        email_id: 'test@example.com',
      });
      expect(bcrypt.compare).toHaveBeenCalledWith(
        'password123',
        mockTrustee.password_hash,
      );
    ...
    it('should throw UnauthorizedException for invalid credentials', async () => {
      ...
      await expect(
        service.loginAndGenerateToken('nonexistent@example.com', 'pass'),
    ...
    it('should throw UnauthorizedException for incorrect password', async () => {
      ...
      await expect(
        service.loginAndGenerateToken('test@example.com', 'wrongpass'),
      ).rejects.toThrow(UnauthorizedException);
    });

    it('should throw UnauthorizedException for unexpected errors', async () => {
      jest
        .spyOn(trusteeModel, 'findOne')
        .mockRejectedValue(new Error('Unexpected error'));

      await expect(
        service.loginAndGenerateToken('test@example.com', 'password123'),
      ).rejects.toThrow(UnauthorizedException);
    });
  });
  ...
  describe('validateTrustee', () => {
    ...
    it('should throw UnauthorizedException for an invalid token', async () => {
      ...
      await expect(service.validateTrustee('invalidToken')).rejects.toThrow(
        UnauthorizedException,
      );
    });
    ...
    it('should throw UnauthorizedException for an invalid or expired token', async () => {
      ...
      await expect(service.validateTrustee('expiredToken')).rejects.toThrow(
        UnauthorizedException,
      );
    });
    ...
    it('should throw UnauthorizedException for an unexpected error', async () => {
      ...
      await expect(() => service.validateTrustee('mockToken')).rejects.toThrow(
        UnauthorizedException,
      );
    });
  ...
  describe('generateSchoolToken', () => {
    ...
    it('should return genrated token', async () => {
      ...
      const result = await service.generateSchoolToken(
        '6504612e617288b970843181',
        password,
        '658e759736ba0754ca45d0c2',
      );
    ...
    it('should throw NotFoundException for non-existing school', async () => {
      ...
      jest
        ...
        .mockResolvedValue({ _id: new mongoose.Types.ObjectId(trusteeId) });
      jest.spyOn(trusteeSchoolModel, 'findOne').mockResolvedValue(null);

      await expect(
        service.generateSchoolToken(schoolId, password, trusteeId),
    ...
    it('should throw UnauthorizedException for invalid trustee password', async () => {
      ...
      jest
        .spyOn(trusteeModel, 'findById')
        .mockResolvedValue({ _id: new mongoose.Types.ObjectId(trusteeId) });
      jest.spyOn(trusteeSchoolModel, 'findOne').mockResolvedValue({
        _id: new mongoose.Types.ObjectId(),
        trustee_id: new mongoose.Types.ObjectId(trusteeId),
      });
      jest.spyOn(bcrypt, 'compare').mockResolvedValue(false);
