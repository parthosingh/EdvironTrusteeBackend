...
import { SchoolSchema, TrusteeSchool, charge_type } from '../schema/school.schema';
...
describe('PlatformChargesController', () => {
    ...
    beforeAll(async () => {
        ...
        platformChargesController = module.get<PlatformChargesController>(PlatformChargesController);
        platformChargeService = module.get<PlatformChargeService>(PlatformChargeService);
    ...
    it("should add MDR", async () => {
        const trusteeSchool = await new trusteeSchoolModel(mockTrusteeSchools).save()

        const res = await platformChargeService.AddPlatformCharge(trusteeSchool._id.toString(), "UPI", "Others", [{
            upto: null,
    ...
    it("should delete MDR", async () => {
        const trusteeSchool = await new trusteeSchoolModel(mockTrusteeSchools).save()
        await platformChargeService.AddPlatformCharge(trusteeSchool._id.toString(), "UPI", "Others", [{
            upto: null,
            charge_type: charge_type.PERCENT,
            charge: 20
        }])
        const res = await platformChargeService.deletePlatformCharge(trusteeSchool._id.toString(), "UPI", "Others");
